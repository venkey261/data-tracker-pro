...
const SavedEntries = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                setDisplayedEntries(data.entries.slice(0, rowsPerPage)); // Display first page
            } catch (error) {
    ...
    useEffect(() => {
        ...
        setDisplayedEntries(savedEntries.slice(startIndex, endIndex));
    }, [page, rowsPerPage, savedEntries]);
    ...
    const handleSaveClick = async (id) => {
        try {
            ...
            const startIndex = (page - 1) * rowsPerPage;
            const endIndex = startIndex + rowsPerPage;
            setDisplayedEntries(data.entries.slice(startIndex, endIndex));
            setEditingId(null);
            setNewValue('');
        } catch (error) {
    ...
    const totalPages = Math.ceil(savedEntries.length / rowsPerPage);
